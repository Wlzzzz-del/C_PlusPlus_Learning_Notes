...
// wlz:过期
// expired
volatile int timerexpired=0;// wlz:volatile 防止变量被编译器优化
int speed=0;
int failed=0;
int bytes=0;
...
int main(int argc, char *argv[])
{
    ...
    if(argc==1)// wlz:如果只传入一个参数
    {
        usage();
        return 2;
    } 

		/*wlz: getopt_long,getopt,getopt_long_only 是GNUC提供的解析命令行参数的函数
				   第三个参数optstring表示短选项
				   其中p\c\?带参数
				   其他选项都不带参数*/
    ...
    build_request(argv[optind]);// wlz: 处理请求头部host\request
    ...
    */
    ...
    return bench();// wlz:return 的时候调用bench
}
